// app/player/[uuid]/characters/[characterUuid]/page.tsx
...
export default async function CharacterDetailPage({
    ...
    try {
        ...
        const abilities = charFull.abilities;

        const classType = charData?.type?.toLowerCase();
        const [abilityMap, abilityTree] = await Promise.all([
            AbilityService.getMap(classType).catch(() => null) as Promise<Record<string, any[]> | null>,
            AbilityService.getTree(classType).catch(() => null) as Promise<any>,
        ]);
        ...
        return (
            <div className="max-w-6xl mx-auto py-12 px-8">
                ...
                <nav className="mb-8 flex items-center gap-2 text-sm font-medium text-black/50">
                    ...
                    <Link href={`/player/${uuid}`}>{playerUsername}</Link>

                    <ChevronRight className="w-4 h-4" />

                    <span className={config.textColor}>{charData.nickname || charData.type}</span>
                    ...
                    <div className="relative z-10 p-10 flex flex-col md:flex-row items-center gap-8">
                        ...
                        <div className="flex-1 text-white text-center md:text-left">
                            ...
                            <p className="text-white/60 text-sm font-bold uppercase tracking-wider mb-6">
                                {playerUsername} tarafından oynanan karakter
                            </p>
                            ...
                            <div className="max-w-md mx-auto md:mx-0">
                                ...
                                <div className="flex items-end justify-between mb-2">
                                    ...
                                    <span className="text-5xl font-jersey leading-none">Lv.{charData.level}</span>

                                    <span className="text-lg font-bold text-white/40">{charData.xpPercent || 0}%</span>
                ...
                <div className="space-y-8">
                                    ...
                                    <div key={key}>
                                        ...
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-sm font-bold text-black/50 uppercase tracking-wider">{info.label}</span>
                                            </div>
                    ...
                    <div className="border border-black p-10">
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            <MiniStat label="Mobs Killed" value={charData.mobsKilled} />
                            <MiniStat label="Deaths" value={charData.deaths} />
                            <MiniStat label="PvP Kills" value={charData.pvp?.kills} />
                            <MiniStat label="PvP Deaths" value={charData.pvp?.deaths} />
                            <MiniStat label="Wars" value={charData.wars} />
                            <MiniStat label="Chests Found" value={charData.chestsFound} />
                            <MiniStat label="Playtime" value={charData.playtime ? `${charData.playtime}h` : null} />
                            <MiniStat label="Blocks Walked" value={charData.blocksWalked} />
                            <MiniStat label="Logins" value={charData.logins} />
                            <MiniStat label="Discoveries" value={charData.discoveries} />
                            <MiniStat label="World Events" value={charData.worldEvents} />
                            <MiniStat label="Caves" value={charData.caves} />
                            <MiniStat label="Lootruns" value={charData.lootruns} />
                            <MiniStat label="Content Completion" value={charData.contentCompletion} />
                            <MiniStat label="Items Identified" value={charData.itemsIdentified} />
                            <MiniStat label="Pre-Economy" value={charData.preEconomy} />
                        </div>
                                    ...
                                    return (
                                        <div key={name} className={`p-5 border ${config.bgAccent} ${config.borderAccent}`}>
                                            ...
                                            <div className="flex items-center justify-between mb-3">
                                                <span className="text-sm font-black text-black uppercase tracking-wider">{name}</span>
                                                <span className="text-2xl font-jersey text-black">Lv.{level}</span>
                                            </div>
                                            ...
                                            <p className="text-xs text-black/50 mt-3">{xpPercent}% XP</p>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {Object.entries(dungeons.list || {}).map(([name, count]: [string, any]) => (
                                    ...
                                    <div key={name} className="flex items-center justify-between p-5 border border-black">
                                        ...
                                        <span className="font-jersey text-2xl text-black/50">{count}</span>
                    ...
                    {abilityMap && (
                        ...
                        <div className="border border-black p-10">
                            ...
                            <AbilityTree
                                ...
                                abilityTreeData={abilityTree}
                                unlockedAbilities={(() => {
                                    if (!abilities) return [];
                                    const arr = Array.isArray(abilities) ? abilities : [];
                                    return arr
                                        .filter((node: any) => node.type === 'ability' && node.meta?.id)
                                        .map((node: any) => node.meta.id);
                                })()}
                                classType={classType}
...
function MiniStat({ label, value }: { label: string; value: any }) {
    return (
        <div className="p-4 border border-black text-center">
            <p className="text-xs font-black text-black/50 uppercase">{label}</p>
            <p className="text-2xl font-jersey text-black leading-none">{value !== undefined && value !== null ? value ?? value : '—'}</p>
        </div>
    );
}